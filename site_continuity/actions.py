...
def prepare_for_failover(name=None, dr_plan_info=None):
    ...
    if response.status_code == 201:
        ...
        response = response.json()
        print(
            'unsuccessful to Prepare For Failover {name}, due to error - {error}'.format(name=dr_plan_info.get('name'),
                                                                                         error=response.get(
                                                                                             'errorMessage')))
...
def prepare_for_failback(name=None, dr_plan_info=None):
    ...
    if response.status_code == 201:
        ...
        response = response.json()
        print(
            'unsuccessful to prepared for failback {name}, due to error - {error}'.format(name=dr_plan_info.get('name'),
                                                                                          error=response.get(
                                                                                              'errorMessage')))


def failback(dr_plan_info=None, dr_plan_name=None, performStorageVmotion=False, test_failback=False,
